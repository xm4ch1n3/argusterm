...
fn load_templates() -> Environment<'static> {
    ...
    for (name, src) in [
        (
            "triage_system",
            include_str!("../../prompts/triage_system.j2"),
        ),
...
async fn extract_ids(
    ...
    let user = deps
        .env
        .get_template("extract_ids")
        .expect("missing extract_ids")
        .render(
            minijinja::context! { title => title, description => description, content => content },
        )
        .expect("failed to render extract_ids");
    match call_json::<ExtractIds>(
        &deps.client,
        &deps.api_key,
        &deps.model_extract,
        "",
    ...
    .await
    {
        Ok(o) => o.cve_ids,
        Err(e) => {
            tlog(entry_id, "ids_err", &format!("{e:#}"));
            Vec::new()
        }
    }
}
...
async fn pick_ref(deps: &LlmDeps, entry_id: &str, nvd_content: &str) -> Option<PickRef> {
    ...
    .await
    {
        Ok(p) => Some(p),
        Err(e) => {
            tlog(entry_id, "pick_err", &format!("{e:#}"));
            None
        }
    }
}
...
async fn summarize(
    ...
    let system_prompt = deps
        .env
        .get_template("triage_system")
        .expect("missing triage_system")
        .render(minijinja::context!())
        .expect("failed to render triage_system");
    let user_msg = deps
        .env
        .get_template("triage_user")
        .expect("missing triage_user")
        .render(minijinja::context! {
            entry_id => &entry.id, entry_title => &entry.title, entry_content => content,
        })
        .expect("failed to render triage_user");
    call_json::<TriageResult>(
        &deps.client,
        &deps.api_key,
        &deps.model_summarize,
        &system_prompt,
        &user_msg,
        TRIAGE_SCHEMA,
        8192,
...
async fn triage_one(
    ...
    tlog(
        ...
        &format!(
            "{} chars ({note})",
            primary.as_ref().map(|s| s.len()).unwrap_or(0)
        ),
    );
        ...
        ([id], Some(key)) => {
            ...
            if let Some((cached_url, cached_content)) = cached {
                tlog(
                    ...
                    "cache",
                    &format!(
                        "hit cve={id} url={cached_url} ({} chars)",
                        cached_content.len()
                    ),
                );
                ...
                tlog(
                    &entry.id,
                    "nvd",
                    &format!("{} chars", nvd.as_ref().map(|s| s.len()).unwrap_or(0)),
                );
